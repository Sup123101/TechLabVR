...
namespace VRWeapons.InteractionSystems.Generic
{
    public class VRW_ObjectSetupGeneric : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button(new GUIContent("Set up controller scripts", "Adds a VRW_GenericIS_Controller script to each SteamVR controller found in the scene." +
                "\nThis script handles controller events, and is required for any weapons in the scene.")))
            {
                MonoBehaviour[] controllers = FindObjectsOfType<SteamVR_TrackedObject>();
                foreach(MonoBehaviour ctrl in controllers)
                {
                    if (ctrl.GetComponent<VRW_GenericIS_Controller>() == null && ctrl.GetComponent<Camera>() == null)
                    {
                        ctrl.gameObject.AddComponent<VRW_GenericIS_Controller>();
                    }
                }
            }
                ...
                "magazines are capable of individual bullet behavior, and require rounds to be set up in the magazine. \n\nIf weapon has an internal magazine, select the weapon and press this.")))
            {
                ...
                if (newMag.GetComponent<VRW_GenericIS_Interactable>() == null && newMag.GetComponent<Weapon>() == null)
                {
