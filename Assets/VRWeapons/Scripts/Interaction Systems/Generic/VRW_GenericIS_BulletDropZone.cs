    ...
    public class VRW_GenericIS_BulletDropZone : MonoBehaviour
    {
        ...
        IEnumerator LerpMovement(Transform t)
        {
            ...
            while (Time.time < startTime + timeToInsert)
            {
                ...
                t.localPosition = Vector3.Lerp(startPos, bulletLocation.localPosition, lerpVal);
                t.rotation = Quaternion.Lerp(startRot, bulletLocation.rotation, lerpVal);
                yield return new WaitForFixedUpdate();
            }
            t.parent = thisMagGO.transform;
            isLerping = false;
        }
